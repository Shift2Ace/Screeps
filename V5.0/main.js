...
var myRoleS = {
    ...
    linkharvester:require('roleS.linkharvester')
};
...
module.exports.loop = function () {
    ...
    for (var name in Game.rooms) {
        ...
        // set the number of tower in memory
        var towers = room.find(FIND_STRUCTURES, {
            filter: (structure) => {
                return (structure.structureType == STRUCTURE_TOWER);
            }
        });
        var numberOfTowers = towers.length;
        room.memory.tower = numberOfTowers;
        
        // increase the number of soldier to 1 if there are enemy
        if(room.memory.creepNeed){
            if(room.find(FIND_HOSTILE_CREEPS).length){
                room.memory.creepNeed.soldier = 1;
            }else{
        ...
        //create empty object for counting creep
        ...
        var pos = 4.2;
        for (var role in room.memory.creepNeed){
            room.visual.text('No. of ' + role + ' : ' + room.memory.creepNeed[role],0,pos,{align: 'left',color: 'green'});
            pos += 1;
        }
    ...
    for (var role in myRole){
        for (var name in Game.rooms){
            ...
            if(Game.rooms[name].memory.creepNeed && Game.rooms[name].memory.creep){
                if (!Game.rooms[name].memory.creepNeed[role]){
                    Game.rooms[name].memory.creepNeed[role] = 0;
                }
